...
const backupSyncStatus = async (req, res) => {
  ...
  try {
    ...
    const [pendingJobs] = await connection.query(
      'SELECT dj.id, dj.job_name, dj.description, dj.job_type, dj.start_time, dj.end_time, dj.status, dj.total_objects, dj.total_records, dj.total_bytes FROM data_transfer_jobs dj JOIN org_drive_mappings m ON dj.mapping_id = m.id JOIN salesforce_orgs s ON m.org_id = s.id WHERE dj.status = "IN_PROGRESS" AND s.org_id = ?LIMIT 1',
      [salesforce_org_id] 
    );
    ...
    if (pendingJobs.length > 0) {
      ...
      const [pendingObjectJobs] = await connection.query(
        "SELECT dol.id, dol.data_transfer_job_id, dol.object_name, dol.fields_count, dol.estimated_size, dol.status, dol.folderId, dol.created_at, dol.updated_at FROM data_transfer_object_log dol JOIN data_transfer_jobs dj ON dol.data_transfer_job_id = dj.id WHERE dol.data_transfer_job_id IN (?) LIMIT 1",
        [jobIds]
      );
